...
const rainySeasonMonthsByCountry = {
  KR: [6, 7],
  JP: [5, 6],
} as const

const rainySeasonCountryLabels = {
  KR: '한국',
  JP: '일본',
} as const

const getRainySeasonCountries = (cityContext: PlannerCityContext | null) =>
  cityContext ? [cityContext.country] : (['KR', 'JP'] as const)

const formatMonthRange = (months: readonly number[]) => `${months[0]}-${months[months.length - 1]}월`

export const getRainySeasonStatus = (
  cityContext: PlannerCityContext | null,
  travelMonth: number | null,
): RainySeasonStatus => {
  const countries = getRainySeasonCountries(cityContext)
  const matchedCountryLabels =
    travelMonth === null
      ? []
      : countries
          .filter((country) => rainySeasonMonthsByCountry[country].includes(travelMonth as never))
          .map((country) => rainySeasonCountryLabels[country])
  const rainyMonthLabel = countries
    .map((country) => `${rainySeasonCountryLabels[country]} ${formatMonthRange(rainySeasonMonthsByCountry[country])}`)
    .join(' · ')
  const isRainySeason = matchedCountryLabels.length > 0
  const selectedMonthLabel = getTravelMonthLabel(travelMonth)

  return {
    selectedMonth: travelMonth,
    isRainySeason,
    matchedCountryLabels,
    rainyMonthLabel,
    basisLabel: `${rainyMonthLabel} 2025 더미 기준`,
    statusLabel:
      travelMonth === null
        ? '월 선택 대기'
        : isRainySeason
          ? `${selectedMonthLabel}은 ${matchedCountryLabels.join('·')} 우기 기준입니다.`
          : `${selectedMonthLabel}은 우기 기준이 아닙니다.`,
  }
}
...
export const createPlanDraft = (
  ...
  const variantLabel = `추천안 ${variantIndex + 1}`
  const variantMoveOffset = variantIndex % 2 === 0 ? 0 : 4
  const variantTitleSuffix = variantIndex === 0 ? '' : ' 다른 흐름'
  const createPlanDayStops = (dayNumber: number): PlanStop[] => {
    const isFirstDay = dayNumber === 1
    const dayRouteOffset = variantIndex + dayNumber - 1

    return [
      {
        time: '아침',
        move: isLessWalking ? `${12 + variantMoveOffset}분` : `${18 + variantMoveOffset}분`,
        title: isFirstDay
          ? isLessWalking
            ? `가볍게 도착하고 가까운 동네부터 보기${variantTitleSuffix}`
            : `가볍게 도착하고 동네 감 잡기${variantTitleSuffix}`
          : `${dayNumber}일차 아침 컨디션 맞추기${variantTitleSuffix}`,
        body: isFirstDay ? getRouteOption(dayRouteOffset, firstRoute) : getRouteOption(dayRouteOffset + 1, firstRoute),
        reason: isFirstDay
          ? cityContext
      ...
      {
        time: '점심',
        move: isLessWalking ? `${16 + variantMoveOffset}분` : `${24 + variantMoveOffset}분`,
        title: isFirstDay
          ? isArtFocused
            ? `전시와 편집숍을 한 동선 안에 묶기${variantTitleSuffix}`
            : `취향에 맞는 핵심 장소 둘러보기${variantTitleSuffix}`
          : `${preference.tag} 테마를 더 깊게 보기${variantTitleSuffix}`,
        body: isFirstDay
          ? cityContext
            ? getRouteOption(dayRouteOffset + 1, middleRoute)
            : isArtFocused
              ? '전시 공간 · 편집숍 · 쉬어가는 카페를 한 구역에 묶어 봅니다.'
              : preference.description
          : getRouteOption(dayRouteOffset + 2, middleRoute),
        reason: isFirstDay
          ? cityContext
      ...
      {
        time: '저녁',
        move: isLessWalking ? `${10 + variantMoveOffset}분` : `${15 + variantMoveOffset}분`,
        title: isFirstDay ? `무리하지 않는 마무리 동선${variantTitleSuffix}` : `${dayNumber}일차를 느슨하게 정리하기${variantTitleSuffix}`,
        body: isFirstDay ? getRouteOption(dayRouteOffset + 2, lastRoute) : getRouteOption(dayRouteOffset + 3, lastRoute),
        reason: isFirstDay
  ...
  const rainySeason = getRainySeasonStatus(cityContext, travelMonth)
  const createRainyStops = (stops: PlanStop[]): PlanStop[] =>
    stops.map((stop) => {
      if (stop.time === '아침') {
        return {
          ...stop,
          move: '8분',
          title: '비 오는 날 실내 거점에서 시작',
          body: `${stop.body} 주변의 실내 전시, 로컬 상점, 카페를 먼저 묶어 이동 노출을 줄입니다.`,
          reason: '우기 더미 판정이 켜진 달에는 야외 첫 동선을 줄이고 젖어도 부담이 작은 실내 거점부터 시작합니다.',
        }
      }
      ...
      if (stop.time === '점심') {
        return {
          ...stop,
          move: '10분',
          title: '시장·식당·전시를 가까운 구역에 묶기',
          body: `${stop.body} 대신 비를 피하기 쉬운 시장, 식당, 실내 관람 후보를 같은 구역 안에서 봅니다.`,
          reason: '중간 시간대는 체류 시간이 길어 비가 오면 실내 체류와 식사 동선을 함께 잡는 편이 안정적입니다.',
        }
      }
      ...
      return {
        ...stop,
        move: '7분',
        title: '숙소권 안에서 우천 마무리',
        body: `${stop.body} 흐름은 유지하되 숙소 주변 카페, 온천, 실내 야경 포인트로 마무리합니다.`,
        reason: '저녁에는 이동을 더 줄이고 귀가 동선을 짧게 만들어 우천 리스크를 낮춥니다.',
      }
    })
  const days: PlanDay[] = Array.from({ length: dayCount }, (_, dayIndex) => {
    const dayNumber = dayIndex + 1
    const stops = createPlanDayStops(dayNumber)
    ...
    return {
      ...
      rainySummary:
        dayNumber === 1
          ? '비가 오는 날은 첫 장소를 실내 거점으로 바꾸고 야외 이동을 짧게 잡습니다.'
          : `${dayNumber}일차 우천 대체안은 실내 체류와 숙소권 복귀를 먼저 봅니다.`,
      rainyStops: createRainyStops(stops),
    }
  })
  ...
  return {
    ...
    variantLabel,
    rainySeason,
    summary: `${baseSummary} ${festivalThemeSummary} ${rainySeason.basisLabel}으로 우천 대체안을 함께 준비합니다.`,
    days,
...
export const createAssistantReply = (
  ...
  `${cityContext ? `${cityContext.cityName} 중심으로` : `${basisLabel} 기준으로`} ${draft.durationLabel} 흐름을 잡아볼게요. ${draft.intensityLabel}으로 ${formatThemeList(extraction.activeRequiredThemes)} 장면을 먼저 배치했습니다.${
    draft.rainySeason.selectedMonth ? ` ${draft.rainySeason.statusLabel}` : ''
  }${
...
export const createPlanId = (
  ...
  [
    ...
    draft.rainySeason.selectedMonth ? getTravelMonthLabel(draft.rainySeason.selectedMonth) : null,
    draft.variantLabel,
