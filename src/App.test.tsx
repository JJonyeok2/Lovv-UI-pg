...
describe('MVP main entry screen', () => {
  ...
  it('opens the AI planner from a selected map city without mutating the stored preference', () => {
    ...
    expect(screen.getAllByText(/6월은 한국 우기 기준입니다/).length).toBeGreaterThanOrEqual(1)
    ...
    expect(screen.getByRole('button', { name: '우천시' })).not.toBeDisabled()
    fireEvent.click(screen.getByRole('button', { name: '우천시' }))
    expect(screen.getByText('우천시 대체 코스를 표시 중입니다.')).toBeInTheDocument()
  ...
  it('keeps dense text responsive on narrow screens', () => {
    ...
    expect(screen.getByRole('heading', { name: '여행 조건 정리' })).toHaveClass(
      'break-keep',
      'max-sm:text-xl',
      'max-sm:leading-7',
    )
    ...
    expect(screen.getByText('평소 기준 코스를 표시 중입니다.')).toHaveClass('break-keep')
  })
  ...
  it('skips onboarding for returning users and opens the chat workspace without a map', () => {
    ...
    expect(screen.getByTestId('chat-top-grid')).toHaveClass('grid-cols-[minmax(0,0.92fr)_minmax(340px,0.72fr)]')
  ...
  it('turns a chat message into an assistant response and updated itinerary detail', () => {
    ...
    fireEvent.change(screen.getByLabelText('여행 월 선택'), { target: { value: '6' } })
    expect(screen.getAllByText(/6월은 한국·일본 우기 기준입니다/).length).toBeGreaterThanOrEqual(1)
    fireEvent.click(screen.getByRole('button', { name: '우천시' }))
    expect(screen.getByText('우천시 대체 코스를 표시 중입니다.')).toBeInTheDocument()
  })
  ...
  it('saves and likes a generated itinerary without duplicate mock storage records', () => {
    ...
    expect(screen.getByRole('heading', { name: '온천·휴양 2박 3일 초안' })).toBeInTheDocument()
    expect(screen.getByText(/기존 조건은 유지하고 추천안 2로 다시 구성했어요/)).toBeInTheDocument()
    expect(screen.getByText('추천안 2')).toBeInTheDocument()
    expect(screen.getByRole('button', { name: '마이페이지에 저장' })).toBeInTheDocument()
    expect(JSON.parse(localStorage.getItem('lovv.savedPlans') ?? '[]')).toHaveLength(1)
  })
