...
function App() {
  ...
  const [planDisplayMode, setPlanDisplayMode] = useState<PlanDisplayMode>('normal')
  const [planVariantIndex, setPlanVariantIndex] = useState(0)
  ...
  const shouldAskRainySeasonMonth = Boolean(plannerCityContext)
  ...
  const shouldShowTravelMonthPrompt =
    ...
    (shouldAskTravelMonthForCity(plannerCityContext, festivalThemeChoice) || shouldAskRainySeasonMonth)
  ...
  const effectivePlanDisplayMode: PlanDisplayMode = planDraft.rainySeason.isRainySeason
    ? planDisplayMode
    : 'normal'
  ...
  const savedRoutePlanDraft = useMemo<PlanDraft | null>(() => {
    ...
    return {
      ...
      variantLabel: savedPlanForRoute.variantLabel ?? '추천안 1',
      rainySeason: savedPlanForRoute.rainySeason ?? getRainySeasonStatus(null, null),
      summary: savedPlanForRoute.summary,
      days,
      stops: savedPlanForRoute.stops,
    }
  ...
  const plannerConditionSummary = plannerConditionExtraction
    ? [
        ...
        selectedTravelMonth ? planDraft.rainySeason.statusLabel : null,
        ...
        ? `${plannerConditionSummary} 조건으로 구성 중입니다.`
          ...
          : shouldShowDurationPrompt
            ...
            : shouldShowTravelMonthPrompt
              ? '지역 우기 여부를 확인하기 위해 여행 예정 월을 선택해 주세요.'
              : '축제 포함 여부를 먼저 골라주세요.',
  ...
  const resetPlannerFlow = (
    ...
    setPlanDisplayMode('normal')
    setPlanVariantIndex(0)
    ...
    setPlanDraft(createPlanDraft(preference, nextPlannerContextText, nextFestivalThemeChoice, cityContext, null, 0))
  ...
  const saveGeneratedPlan = () => {
    ...
    const nextPlan: SavedPlan = {
      ...
      variantLabel: planDraft.variantLabel,
      rainySeason: planDraft.rainySeason,
  ...
  const selectPlannerTravelMonth = (month: number) => {
    if (selectedDurationLabel === null) {
      return
    }

    const currentMonthLabel = selectedTravelMonth ? getTravelMonthLabel(selectedTravelMonth) : ''
    const baseContextText = currentMonthLabel
      ? plannerContextText.replace(currentMonthLabel, '').replace(/\s+/g, ' ').trim()
      : plannerContextText
    const nextPlannerContextText = `${baseContextText} ${getTravelMonthLabel(month)}`.trim()
  ...
  const regeneratePlannerResult = () => {
    if (!isPlannerReady || selectedDurationLabel === null) {
      resetPlannerFlow()
      return
    }

    const nextVariantIndex = planVariantIndex + 1
    const nextExtraction =
      plannerConditionExtraction ??
      createMockConditionExtraction(
        '',
    ...
    const nextDraft = createPlanDraft(
      plannerPreference,
      `${selectedDurationLabel} ${plannerContextText}`.trim(),
      festivalThemeChoice,
      plannerCityContext,
      selectedTravelMonth,
      nextVariantIndex,
    ...
    setPlanVariantIndex(nextVariantIndex)
    setPlanDisplayMode('normal')
    setPlannerConditionExtraction(nextExtraction)
    setPlanDraft(nextDraft)
    setSelectedPlanDayNumber(1)
    ...
    setChatMessages((currentMessages) => [
      ...currentMessages,
      {
        ...
        role: 'assistant',
        content: `기존 조건은 유지하고 ${nextDraft.variantLabel}로 다시 구성했어요.`,
      },
    ])
  }
  ...
  const submitChatMessage = (message: string) => {
    ...
    if (selectedDurationLabel === null) {
      ...
      const assistantContent = shouldAskTravelMonth
        ? '여행 예정 월을 골라주세요. 선택한 지역의 우기 더미 기준을 함께 확인할게요.'
        : nextExtraction
        ...
        <>
          ...
          {activeView === 'home' ? (
            ...
            <PlanDetailView
              ...
              planDisplayMode={effectivePlanDisplayMode}
              onPlanDisplayModeChange={setPlanDisplayMode}
            ...
            <PlannerWorkspace
              ...
              onSelectTravelMonth={selectPlannerTravelMonth}
              ...
              regeneratePlannerResult={regeneratePlannerResult}
              saveGeneratedPlan={saveGeneratedPlan}
